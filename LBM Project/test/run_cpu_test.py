...
if __name__ == '__main__':
    ...
    for mytime in range(0,timestep):
        ...
        t1=time.time()
        run('collision','cpu',u=u,v=v,f=f,feq=feq,rho=rho,omega=omega,w=w,cx=cx,cy=cy,n=n,m=m,th=th,gbeta=gbeta)
        t2 = time.time()
        run('streaming','cpu',f=f, n=n,m=m)
        t3 = time.time()
        run('bounceb','cpu',f=f, n=n,m=m)
        t4 = time.time()
        run('rhouv','cpu',f=f,rho=rho,u=u,v=v,cx=cx,cy=cy,n=n,m=m)
        t5 = time.time()
        run('collt','cpu',u=u, v=v, g=g, geq=geq, th=th, omegat=omegat, w=w, cx=cx, cy=cy, n=n,m=m)
        t6 = time.time()
        run('streaming','cpu',f=g,n=n,m=m)
        t7 = time.time()
        run('gbound','cpu',g=g,tw=tw,w=w,n=n,m=m)
        t8 = time.time()
        run('tcalcu','cpu',g=g,th=th,n=n,m=m)
        t9 = time.time()
